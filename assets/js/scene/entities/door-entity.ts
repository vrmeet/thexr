...
export class DoorEntity extends EntityBase {
    ...
    getWallMiddle(globalPoints: BABYLON.Vector3[]): { middleX: number, middleZ: number } {
        const minsAndMaxes = globalPoints.reduce((acc, point) => {
            if (point.x > acc.maxX) {
                acc.maxX = point.x
            }
            if (point.x < acc.minX) {
    ...
    emitCreateEntityEvent(opts: { globalPoints: BABYLON.Vector3[] }) {
        ...
        const offsetPosition = new BABYLON.Vector3(middle.middleX, 0, middle.middleZ)
        const pointsInLocalSpace = opts.globalPoints.map(globalPoint => globalPoint.subtract(offsetPosition))
    ...
    createWall = (points: number[], height: number = 2) => {
        ...
        from(points).pipe(
            ...
            let wall = BABYLON.MeshBuilder.CreateBox("", { width: 0.1, depth: length, height }, this.scene)
            parts.push(wall)
            let c1;
            // if this is the first wall, make a tube at the end
            if (data.partCount === 1) {
                c1 = BABYLON.MeshBuilder.CreateCylinder("", { height: height, diameter: 0.1 }, this.scene)
                c1.position.copyFromFloats(point1.x, height / 2, point1.z)
                parts.push(c1)
            }

            let c2 = BABYLON.MeshBuilder.CreateCylinder("", { height: height, diameter: 0.1 }, this.scene)
    ...
    createMesh() {
        const { height, points } = this.argifyComponents(this.components, ["height", "points"])
