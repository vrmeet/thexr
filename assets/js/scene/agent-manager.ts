...
export class AgentManager {
  ...
  maxAgentPoolSize() {
    return (
      member_states.membersCount() *
      this.getAllEnemySpawnPoints().length *
      this.waveNumber
    );
  }
  ...
  subscribeToAgentHitEvent() {
    signalHub.incoming
      .on("event")
      .pipe(filter(evt => evt.m === EventName.agent_hit))
      .subscribe(evt => {
    ...
    if (mode.leader) {
      if (this.state === "spawning") {
        ...
        if (this.agentsCount() === 0) {
          signalHub.outgoing.emit("hud_broadcast", {
            msg: `Wave ${this.waveNumber} Passed`,
          });
          this.state = "wave_ended";
        }
  ...
  beginNewWave() {
    this.waveNumber++;
    signalHub.outgoing.emit("hud_broadcast", {
      msg: `Starting Wave ${this.waveNumber}`,
    });
    this.state = "spawning";
  }
  ...
  subscribeToAgentCreatedEvent() {
    signalHub.incoming
      .on("event")
      .pipe(filter(evt => evt.m === EventName.agent_spawned))
      .subscribe((evt: any) => {
  ...
  createAgentEvent(position: BABYLON.Vector3) {
    this.preemptiveCount++;
    let evt: event = {
      m: EventName.agent_spawned,
      p: { name: `agent_${random_id(5)}`, position: position.asArray() },
    };
