...
export class BulletManager {
  ...
  public targets: {
    [entity_id: string]: {
      origPosition: BABYLON.Vector3;
      origRotation: BABYLON.Quaternion;
      resetCallback: any;
    };
  };
  ...
  constructor(public my_member_id: string, public scene: BABYLON.Scene) {
    ...
    signalHub.local.on("trigger_squeezed_with_entity").subscribe(payload => {
      if (this.myAmmoCount > 0) {
        ...
        let event: event = {
          ...
          p: { member_id: this.my_member_id, ...payload },
        };
    ...
    signalHub.incoming
      ...
      .pipe(
        filter(
          evt =>
            evt.m === EventName.entity_collected &&
            evt.p.member_id === this.my_member_id
        )
      )
      .subscribe(evt => {
        ...
        if (
          collectedMesh &&
          collectedMesh.metadata &&
          collectedMesh.metadata.ammo
        ) {
    ...
    signalHub.incoming
      .on("event")
      .pipe(filter(event => event.m === EventName.entity_trigger_squeezed))
      .subscribe(mpts => {
        this.fireBullet(
          mpts.p["member_id"],
          mpts.p["pos"],
    ...
    signalHub.incoming
      .on("event")
      .pipe(filter(event => event.m === EventName.target_hit))
      .subscribe(event => {
  ...
  fireBullet(
    ...
    let bullet = BABYLON.MeshBuilder.CreateCapsule(
      "bullet",
      {
        height: size * 0.1,
        radiusTop: size * 0.03,
        radiusBottom: size * 0.05,
      },
      this.scene
    );
    // set bullet position
    ...
    let target = bullet.position.add(
      BABYLON.Vector3.FromArray(direction).scale(distance)
    );
    ...
    let checkBulletForIntersect = () => {
      ...
      if (hitTest.pickedMesh) {
        ...
        if (this.my_member_id === bullet_owner_member_id) {
          ...
          if (
            hitTest.pickedMesh.metadata != null &&
            hitTest.pickedMesh.metadata["member_id"] != undefined
          ) {
            signalHub.outgoing.emit("event", {
              m: EventName.member_damaged,
              p: { member_id: hitTest.pickedMesh.metadata["member_id"] },
            });
          } else if (this.pickedMeshIsAgent(hitTest.pickedMesh)) {
            const payload: event = {
              m: EventName.agent_hit,
              p: {
                name: hitTest.pickedMesh.metadata["agentName"],
                pos: hitTest.pickedPoint.asArray(),
                direction: direction,
              },
            };
            ...
            <string[]>(
              BABYLON.Tags.GetTags(hitTest.pickedMesh)?.includes("targetable")
            )
          ) {
            const payload: event = {
              m: EventName.target_hit,
              p: {
                entity_id: hitTest.pickedMesh.id,
                pos: hitTest.pickedPoint.asArray(),
                direction: direction,
              },
            };
  ...
  affectTargetable = (
    ...
    if (!this.targets[pickedMesh.id]) {
      ...
      if (!pickedMesh.physicsImpostor) {
        pickedMesh.physicsImpostor = new BABYLON.PhysicsImpostor(
          pickedMesh,
