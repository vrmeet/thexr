...
export class GameConstructs extends GUI.Container {
  ...
  constructor(public scene: BABYLON.Scene) {
    ...
    let options = [
      a({ callback }, "< Tools"),
      this.scrollableConstructOptions(),
    ];

    this.addControl(div({ name: "constructs-container" }, ...options));
  }
  ...
  dropOptions() {
    const options = {
      ...
      enemy_spawner: EnemySpawnerEntity,
      ammo_box: AmmoBoxEntity,
      gun: GunEntity,
    };
  ...
  */
  ...
  scrollableConstructOptions() {
    const gotoBarrierMaker = () => {
      signalHub.menu.emit("menu_topic", "barriermaker");
    };
    const dropAmmoBox = () => {
      let components = {
        color: "#F002A3",
      ...
      const entity_event: event = {
        ...
        p: {
          type: "ammo_box",
          id: uuidv4(),
          name: `ammo_box_${random_id(5)}`,
          components: this.componentObjToList(components),
        },
      };
    ...
    return pre(
      ...
      a(
        { name: "goto-wall-maker", callback: gotoBarrierMaker },
        "Barrier Maker"
      ),
      ...this.dropOptions()
    );
