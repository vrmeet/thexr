...
export class BarrierMaker extends GUI.Container {
  ...
  constructor(public scene: BABYLON.Scene) {
    ...
    this.referenceIndicator = BABYLON.MeshBuilder.CreateCylinder("", {
      height: 1,
      diameter: 0.1,
    });
    ...
    let options = [
      a({ callback }, "< Game Constructs"),
      this.scrollablePrimOptions(),
    ];

    this.addControl(div({ name: "primitives-container" }, ...options));

    let sub = signalHub.local
      .on("pointer_info")
      .pipe(
        ...
        filter(
          info =>
            ...
            info.pickInfo.pickedMesh.metadata?.menu !== true
        ),
      ...
      .subscribe(point => {
        ...
        if (this.wallPoints.length > 1) {
          ...
          this.lines = BABYLON.MeshBuilder.CreateDashedLines(
            "wallLines",
            lineOpts
          );
        }
  ...
  scrollablePrimOptions() {
    const createWallEntity = () => {
      if (this.wallPoints.length < 2) {
        signalHub.incoming.emit(
          "hud_msg",
          "You need at least 2 points to build a wall"
        );
        return;
      }
      signalHub.incoming.emit("hud_msg", `Created Wall`);

      new WallEntity(this.scene).emitCreateEntityEvent({
        globalPoints: this.wallPoints,
      });
      this.reset();
    };
