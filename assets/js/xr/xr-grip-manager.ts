...
export class XRGripManager {
  ...
  constructor(
    ...
    motionController.onModelLoadedObservable.add(() => {
      ...
      this.palmMesh.position =
        this.hand[0] === "l"
          ? new BABYLON.Vector3(0.03, -0.05, 0.0)
          : new BABYLON.Vector3(-0.03, -0.05, 0.0);
      this.palmMesh.rotationQuaternion = BABYLON.Quaternion.FromEulerAngles(
      ...
      signalHub.movement.on(`${this.hand}_grip_released`).pipe(
        tap(() => {
          ...
          if (this.intersectedMeshTags.includes("physics")) {
            event.p.lv = utils.arrayReduceSigFigs(
              this.imposter.getLinearVelocity().asArray()
            );
            event.p.av = utils.arrayReduceSigFigs(
              this.imposter.getAngularVelocity().asArray()
            );
          }
          signalHub.outgoing.emit("event", event);
          signalHub.incoming.emit("event", event);
  ...
  createEventPayload() {
    const payload = {
      ...
      hand_pos_rot: {
        ...
        rot: utils.arrayReduceSigFigs(
          this.palmMesh.absoluteRotationQuaternion.asArray()
        ),
      },
      entity_pos_rot: {
        pos: utils.arrayReduceSigFigs(
          this.intersectedMesh.absolutePosition.asArray()
        ),
        rot: utils.arrayReduceSigFigs(
          this.intersectedMesh.absoluteRotationQuaternion.asArray()
        ),
      },
      hand: this.hand,
    };
