...
export class TintOverlay {
    ...
    constructor(public member_id: string, public scene: BABYLON.Scene) {
        ...
        signalHub.incoming.on("event").pipe(
            filter(msg => (msg.m === EventName.member_died && msg.p.member_id === this.member_id))
        ).subscribe(() => {
            this.tint.pipe(
        ...
        signalHub.incoming.on("event").pipe(
            filter(msg => (msg.m === EventName.member_respawned && msg.p.member_id === this.member_id))
        ).subscribe(() => {
            console.log("respawned", this.tint.value)
            this.tintDown()
        })
    ...
    tintUp(color: BABYLON.Color3, speed: number = 0.03) {
        if (this.tint.value !== "none") {
            return false
        }

        var tintPostProcess = new BABYLON.PostProcess("tintPostProcess", "tint", ["tintColor", "tintAmount"], null, 1, this.scene.activeCamera);
        this.tintAmount = 0
